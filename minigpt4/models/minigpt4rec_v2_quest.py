...
class MiniGPT4Rec_v2(Rec2Base):
    ...
    def __init__(
        ...
        if quest_config is not None and quest_config.enable:
    ...
    def recprompt_wrap_v2(self, samples, ori_samples, atts_sample, prompt): # used for stage 2
        if prompt:
            ...
            print(f"text_len:{[len(prompt_) for prompt_ in prompt_list]}",file=open('text_lens.txt','a'))
    ...
    def forward_v2(self, samples):
        ...
        print(f'token_len:{inputs_embeds.shape[1]}',file=open('token_lens.txt','a'))
        with self.maybe_autocast():
    ...
    def generate_for_samples_v2(self, samples,return_all=False):
        ...
        # logits = outputs.logits[:,-t_posi,:][:,[neg_ans_id, pos_ans_id]]
        ...
        # logits_ = torch.nan_to_num(logits_, nan=-100)

        # logits_ = torch.sigmoid(logits_)

        # print(lo)
        # print('logits:',logits_)
        # print('label:',samples['label'].float())
