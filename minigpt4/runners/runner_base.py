...
class RunnerBase:
    ...
    def __init__(self, cfg, task, model, datasets, job_id):
        ...
        self.start_epoch = 0
        self.train_splits = self._train_splits()
        # self.setup_seeds()
    ...
    def dataloaders(self) -> dict:
        ...
        if self._dataloaders is None:
            ...
            train_ratios = [1]
            if 'reason' in self.train_splits:
                if 'train' in self.train_splits:
                    datasets['reason'].extend(datasets['train'])
                    if self.train_splits[0] == 'reason':
                        train_ratios = self.train_ratios
                    else:
                        train_ratios = self.train_ratios[::-1]
                datasets['train'] = datasets['reason']
                datasets.pop('reason')
            ...
            if self.evaluate_only:
                sample_ratios = [[1]*len(self.datasets[split])  for split in split_names]
            else:
                sample_ratios = [[1] if split != 'train' else train_ratios for split in split_names]
    ...
    @property
    def train_ratios(self):
        train_ratios = self.config.run_cfg.get("train_ratios", [])

        if len(train_ratios) == 0:
            logging.info("Empty train ratios.")
        return train_ratios
    ...
    def setup_output_dir(self):
        ...
        dir_name = self.job_id
        output_dir = lib_root / self.config.run_cfg.output_dir / dir_name
        while os.path.exists(output_dir):
            dir_name = f'{dir_name}_'
            output_dir = lib_root / self.config.run_cfg.output_dir / dir_name
    ...
    def train(self):
        ...
        self.id2title = list(self.datasets.values())[0]['train'].get_id2title()
        ...
        if not self.evaluate_only:# with training
            for cur_epoch in range(self.start_epoch, self.max_epoch):
                    ...
                    for split_name in self.valid_splits:
                        ...
                        if val_log is not None:
                            if is_main_process():
                                ...
                                uauc = val_log["uauc"][0]
                                auc_uauc = agg_metrics/4 + uauc/3
                                if auc_uauc > best_auc_uauc and split_name == "valid":
                                    best_epoch_both, best_auc_uauc = cur_epoch, auc_uauc
                                    self._save_checkpoint(cur_epoch, is_best=True, tag='auc_uauc')
                                    not_change = 0
                ...
                if not self.model_to_betrained():
                    break
                if not_change > 15:
                    logging.info("Early stop. The results has not changed up to 15 epochs.")
                    break
    ...
    def create_loaders(
        ...
        for dataset, bsz, is_train, collate_fn, dataset_ratio in zip(
            datasets, batch_sizes, is_trains, collate_fns, dataset_ratios
        ):
            if isinstance(dataset, list) or isinstance(dataset, tuple):
                if hasattr(dataset[0], 'sample_ratio') and dataset_ratios is None:
                    dataset_ratio = [d.sample_ratio for d in dataset]
                loader = MultiIterLoader(
                    loaders=[
                        _create_loader(d, num_workers, bsz, is_train, collate_fn[i])
                        for i, d in enumerate(dataset)
                    ],
                    ratios=dataset_ratio,
                )
            else:
    ...
    def log_stats(self, stats, split_name):
        if isinstance(stats, dict):
            log_stats = {**{f"{split_name}_{k}": f"{v}" for k, v in stats.items()}}
