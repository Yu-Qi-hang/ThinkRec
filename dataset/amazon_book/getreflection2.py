...
if __name__ == '__main__':
    ...
    parser.add_argument('--api_key', nargs="+", type=str, default=['sk-yhlekcjeoqlqajykctfhgvegkfsffutnfcogetfxpizdlgvo','sk-bvzykhzeyxtiwysnwkdnfgccudpytjjxxndypbemdykqdwad','sk-vdkjbqapbfanjqlrhtrugrslgvdmmhwswgalcezlbkhkudss'])
        ...
        for idx in range(len(batch_data)):
            ...
            while label2text[label] not in answer:
                ...
                if len(answer) == 0 and max_tokens < 3000:
                    ...
                    his_message.append({"role": "user", "content": hints[0].replace('<answer>', label2text[label]).replace('<TargetItemTitle>', target)})
                else:
                    his_message.pop()
                    his_message.append({"role": "user", "content": hints[cnt % 6].replace('<answer>', label2text[label]).replace('<TargetItemTitle>', target)})
                answer, reasoning_content = get_response(idx % len(api_key), his_message, max_tokens)
                cnt += 1
